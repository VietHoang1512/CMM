...
from functorch import  vjp,jvp,make_functional_with_buffers
...
class LinearizedModel(nn.Module):
    ...
    def __init__(self, model: nn.Module, init_model: nn.Module = None) -> None:
        ...
        for p in self.params:
            ...
            if p.shape==torch.Size([768, 512]):
                print("+1 trainable param")
    ...
    def replace_params(self):
        for p, p0 in zip(self.params, self.params0):
    ...
    def get_trainable_params(self):
        for p in self.params:
            if p.requires_grad and p.shape==torch.Size([768, 512]):
                return p
class LinearizedImageEncoder(abc.ABC, nn.Module):
    ...
    def __init__(
        ...
        # self.freeze_lang = image_encoder.freeze_lang
    ...
    def load(cls, filename):
        ...
        # print("Got", state_dict.keys())
        # print("Expecting", taylorized_encoder.state_dict().keys())
        # check if keys match
        # print(taylorized_encoder.state_dict().keys() == state_dict.keys())
        # for key in state_dict.keys():
        #     print(key, state_dict[key].shape, taylorized_encoder.state_dict()[key].shape)
        taylorized_encoder.load_state_dict(state_dict)
        return taylorized_encoder
    def replace_params(self):
        self.model.replace_params()
    def freeze_backbone(self):
