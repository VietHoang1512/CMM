...
def finetune(args, eval_0shot=False, only_eval_0shot=False):
    ...
    if args.task_idx>0:
        ...
        for i, batch in enumerate(data_loader):
            ...
            optimizer.step()                
            wandb.log({
                f"task_{args.task_idx}_representation_loss": loss.item(),
    ...
    if not args.skip_eval:
        _full_r = eval_single_dataset(image_encoder, train_dataset_name, args)['top1']
        wandb.log({f'full_acc': _full_r * 100.0, "task_idx":args.task_idx})
        ...
        for domain in dataset_class.DOMAINS:
            ...
            wandb.log({f'{domain}_acc': _r * 100.0, "task_idx":args.task_idx})
    ##################
...
if __name__ == '__main__':
    ...
    args.save = "outputs/dil/linear/linear-interpolate/" + str(args).replace(", ", "/").replace(
        "'", ""
