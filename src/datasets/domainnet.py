...
class _DomainNet:
    ...
    #                 source_cls_path = os.path.join(self.fpath, domain, 'train' if train else 'test', cls)
    ...
    def __init__(self, 
                 ...
                 subset_config=None):
        """
        :param root: Root directory where <domain>_train.txt / <domain>_test.txt live
        :param train: If True, use <domain>_train.txt, else <domain>_test.txt
        :param transform: Optional transform to be applied on the image
        :param target_transform: Optional transform to be applied on the label
        :param subset_config: 
           - If not None, must have 'domains' key listing which domains to load.
           - If None, load from all known domains in self.DOMAINS.
        """
        super().__init__()

        self.root = os.path.abspath(root + "/domainnet/") 
        ...
        if subset_config is None:
            domains_to_load = self.DOMAINS
        else:
            domains_to_load = subset_config.get('domains', self.DOMAINS)

        # Accumulate all samples from each domain text file
        self.samples = []
        for domain in domains_to_load:
            list_file = f"{domain}_{'train' if train else 'test'}.txt"
            txt_path = os.path.join(self.root, list_file)
            if not os.path.exists(txt_path):
                raise FileNotFoundError(f"Expected file not found: {txt_path}")

            # Read lines from <domain>_train.txt or <domain>_test.txt
            with open(txt_path, 'r') as f:
                for line in f:
                    ...
                    # e.g. "clipart/aircraft_carrier/clipart_001_000018.jpg"
                    # Parse the class name from the second folder
                    ...
                    if len(parts) < 2:
                        ...
                        class_name = parts[1]
                    # Assert that the class name is in the allowed list.
                    assert class_name in self.__class__.CLASSES, \
                        f"Class name '{class_name}' not in allowed classes: {self.__class__.CLASSES}"

                    full_path = os.path.join(self.root, rel_path)
                    self.samples.append((full_path, class_name))

        # Build a list of unique class names, map each to an integer
        class_set = sorted(list(set(s[1] for s in self.samples)))
        self.class_to_idx = {c: i for i, c in enumerate(class_set)}
